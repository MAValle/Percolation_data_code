...
# Creation date: 13-mar-19
# file name: percolation_lattice_v2.R
...
for (probab in seq_along(1:length(probs) ) ) {
  ...
  for (n in seq_along(1:N) )  {
    ...
    result[n, ] <- c(value, dg, quantile(cluster_sizes$csize, probs = c(0.25, 0.5, 0.75)), g_r) 
...
save(MyResults, file="results_from_percolation_lattice_v2_160319c.Rda")
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
ggplot(MyResults, aes(x=k, y=p50) ) +
  geom_point() + 
  scale_shape_manual(values=c(3)) +
  xlab("k") + 
  ylab("<S>") +
...
#user       system  elapsed 
#4413.943   37.028 1184.308  = 19.7 min
# 25-mar-19 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
