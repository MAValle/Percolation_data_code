# Lo mismo que en percolation_ERnetwork_v2.R, pero ahora esta es la 
# version paralelizada para que sea mas rapido.


# Creation date: 26-mar-19
# file name: percolation_ERnetwork_v3.R
...
# Main process:
# Modelo de un graph ER random graph model G(n,q) n es numero de nodos y q es prob de 
...
load("results_from_percolation_ERnetwork_220319.Rda") # resultados con n = 50 nodos
load("results_from_percolation_ERnetwork_220319b.Rda") # resultados con n = 70 nodos
load("results_from_percolation_ERnetwork_220319c.Rda") # resultados con n = 100 nodos

# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# Funcion interna para poder paralelizar el proceso
...
N <- 1000
#probs <- seq(0.1,0.9,0.025) # este es el original para n=10 
#probs <- c(seq(0.01,0.3,0.01), seq(0.3,0.9, 0.2)) # para n=50
probs <- c(seq(0.01,0.1,0.005), seq(0.1,0.9, 0.3)) # para n=100
...
MyResults$S <- MyResults$p50/num_vertex #Normalized cluster size
save(MyResults, file="results_from_percolation_ERnetwork_220319c.Rda")
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 



# Graph 1: probability of connection p versus mean of size of clusters
gr <- ggplot(MyResults, aes(x=probability, y=S)) +
  geom_errorbar(aes(ymin=p25/num_vertex, ymax=p75/num_vertex), width=.007) + 
  geom_point() + 
