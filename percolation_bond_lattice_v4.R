...
# similar a lo que hacemos en percolation_bond_lattice_v3.R
# Esta vez lo que hacemos es dejar activos todos los estados del latice
# (lo cual aqui equivale a dejar disponible todos los productos en las
# gondolas) y simulamos acoples J con cierta distribucion (puede ser
# normal como spin glass o custom con pocoa svalores de J altos y muchos
# valores de J bajos). Luego calculamos los stregth de cada nodo.
# luego simulamos la percolacion y fomracion de clusters eliminando
# consecutivamente los nodos de mayor strength a menor, viceversa y 
# en forma aleatoria para estudiar la formacion y tamano de los cluster
# que representarian canastas de compra.



# step 1: Create a lattice network of LXL with L^2 vertices.
# step 2: Assign to each edge of the lattice a distribution of J values
# step 3: identify the strength of each vertice on the lattice 
# step 4: start simulation connecting each neigbourhood vertex with probability p = 1 - exp(_2J)
#         with all vertex actives (s=1,1,1,,,,,,1)
# step 5: measure observables 
# step 6: drop or leave inactive one of the vertex of the lattice according to some rule and go to step 4.
# step 7: repeat step 4 until some proportion p of the vertex have been removed.
# step 8: repeat again from step 4 N times.


# Creation date: 21-abr-19
# file name: percolation_bond_lattice_v4.R

# Notes:
# 12-abr-19: creation 
# 17-abr-19: repetimos el proceso muchas veces de estudiar los cluster que se forman
#           para un estado particular s.
# 21-abr-19: terminamos de arreglar el loop principal para ir generando todas 
# las simulaciones. El dropeo de nodos es en forma consecutiva desde el nodo con 
# mayor strength al menor strength. Falta cambiar esto para que sea al reves.
...
#J0 <- J <- rep(1, number_of_edges)
# Distribucion de los acoples: random [0,1]
#J0 <- J <- runif(number_of_edges)
# Distribucion de los acoples: random [-1,1]
#J0 <- J <- runif(number_of_edges)*2-1
#J[J < 0] <- 0
# custom discrete tistribution of J
J0 <- J <- sample(x = c(0.1,2), number_of_edges, replace = T, prob = c(0.8, 0.2))
# Normal distribution of J truncated between 0 and 2
library(truncnorm)
#J0 <- J <- rtruncnorm(number_of_edges, a=0, b=2, mean=0.5, sd=0.5)
...
# number of time we remove one vertex at a time
fraction <- 0.9
...
almacen <- matrix(NA, ncol=6, nrow=K) #<S>, max(S), g(r), <k>, <E cluster>, <max E cluster>
...
for (it in 1:K) {
  ...
  #creation of the lattice with dropped vertexs (except for the firs iteration)
  new_lattice <- create_new_lattice_bonds(edg, g)
  new_g <- new_lattice[[1]]
  new_edg <- new_lattice[[2]]
